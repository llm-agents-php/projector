...
MAKE_LOGFILE = /tmp/internal-ai.log
MAKE_CMD_COLOR := $(BLUE)
...
help: ## Show this menu
	...
	@echo
	@echo '    📦 Package                 AI assistant (github.com/llm-agents-php/assistant)'
	@echo '    🏢 ${YELLOW}Org                     PHP Internal (github.com/llm-agents-php)${RST}'
...
phar:
	$(APP_RUNNER) sh -c "git config --global --add safe.directory /app \
		&& .phive/box validate \
		&& .phive/box compile \
		&& .phive/box info .build/phar/ai.phar \
		&& .build/phar/ai.phar"
.PHONY: phar
